              ...
              <td>
                <div style="display:flex;gap:4px;flex-wrap:wrap;">
                  <?php if ($a['status'] === 'Pending'): ?>
                    <button class="btn btn-success btn-sm" onclick="(async ()=>{ if(!confirm('Confirm this appointment?')) return; await updateAppointmentStatus(<?= (int)$a['id'] ?>,'Confirmed'); })()">Confirm</button>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (btn) {
    btn.addEventListener('click', async function (e) {
      ...
      const res = await updateAppointmentStatus(id, 'Cancelled', reason);
      if (res.ok) {
        closeCancelModal();
      } else {
        alert('Failed to cancel: ' + (res.error || 'Unknown'));
      }
    });
  }
});
