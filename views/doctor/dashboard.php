  ...
  <div class="container">
    ...
    <div class="card">
      <div class="card-header">
        <h2 class="card-title">📅 Weekly Grid — Mon–Fri</h2>
        <p class="text-muted" style="margin:0;">Click a slot to expand details or update status.</p>
      </div>
      ...
      <?php
        ...
        for ($d = 0; $d < 5; $d++) {
          ...
          $days[] = [ 'date' => $date, 'label' => date('D M j', strtotime($date)), 'day_name' => date('l', strtotime($date)) ];
        }

        // map appointments by date and time (HH:MM)
        $grid = [];
        foreach ($weekAppointments as $a) {
          $d = $a['appointment_date'];
          $t = substr($a['appointment_time'], 0, 5);
          $grid[$d][$t] = $a;
        }
      ?>
        ...
        <div class="weekly-grid">
          <div class="days-header">
            <?php foreach ($days as $day): ?>
              <div class="day-col"><?= sanitize($day['day_name']) ?><br><small class="text-muted"><?= sanitize($day['label']) ?></small></div>
            <?php endforeach; ?>
          </div>
              ...
              <?php foreach ($days as $day): ?>
                <div class="grid-cell">
                  <?php if (!empty($grid[$day['date']][$s])): $ap = $grid[$day['date']][$s]; ?>
                    <div class="weekly-appt badge-<?= strtolower(str_replace(' ', '-', $ap['status'])) ?>" onclick="(function(el){ const id=<?= (int)$ap['id'] ?>; const info = `<?= sanitize($ap['patient_name']) ?> — <?= sanitize($ap['reason']) ?>`; alert(info); })(this)">
                      <div style="display:flex;gap:8px;align-items:center;">
                        <div class="appt-time"><?= $s ?></div>
                        <div class="appt-patient"><?= sanitize($ap['patient_name']) ?></div>
                      </div>
                      <div style="display:flex;gap:6px;align-items:center;">
                        <button class="btn btn-sm" onclick="(async (e)=>{ e.stopPropagation(); if(!confirm('Mark Completed?')) return; await updateAppointmentStatus(<?= (int)$ap['id'] ?>,'Completed'); })(event)">✓</button>
                        <button class="btn btn-sm" onclick="(async (e)=>{ e.stopPropagation(); if(!confirm('Mark No-Show?')) return; await updateAppointmentStatus(<?= (int)$ap['id'] ?>,'No-Show'); })(event)">NS</button>
                      </div>
                    </div>
                  <?php endif; ?>
                </div>
              ...
              <td>
                <div style="display:flex;gap:6px;align-items:center;">
                  <?php if (!in_array($a['status'], ['Completed','Cancelled'])): ?>
                    <button class="btn btn-success btn-sm" onclick="(async ()=>{ if(!confirm('Mark this appointment as Completed?')) return; await updateAppointmentStatus(<?= (int)$a['id'] ?>,'Completed'); })()">Mark Completed</button>
                    <button class="btn btn-warn btn-sm" onclick="(async ()=>{ if(!confirm('Mark this appointment as No-Show?')) return; await updateAppointmentStatus(<?= (int)$a['id'] ?>,'No-Show'); })()">Mark No-Show</button>
